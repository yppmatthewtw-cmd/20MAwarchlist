#!/usr/bin/env python3
"""R7 series: full US universe from the Nasdaq-snapshot mirror, through the
2026-09-01 close, with the 08-31 session (the mirror was down that day) filled
from the natezone/market-tracker daily bars where that mirror covers the name."""
...
log = subprocess.run(["git", "-C", REPO, "log", "origin/main", "--format=%H|%aI|%s"],
...
for dd in snap_dates:
    _, sha = by_date[dd]
    for path in ("tickers/all.csv", "tickers/sp500.csv", "data/v2/tickers.csv"):
...
# ---- fill the 08-31 session from the daily-bar mirror (snapshot mirror was down) ----
filled = 0
nzdir = f"{NZ}/data/UNIFIED/history"
if os.path.isdir(nzdir):
    import glob as _glob
    for p in _glob.glob(f"{nzdir}/*.csv"):
        sym = os.path.basename(p)[:-4]
        if sym not in close or FILL_DATE in close[sym]:
            continue
        try:
            with open(p, newline="") as f:
                for row in csv.DictReader(f):
                    if (row.get("Date") or "")[:10] == FILL_DATE:
                        c = float(row["Close"]); v = float(row["Volume"] or 0)
                        if c > 0:
                            close[sym][FILL_DATE] = c
                            vol.setdefault(sym, {})[FILL_DATE] = v
                            filled += 1
                        break
        except (OSError, ValueError, KeyError):
            continue
print(f"{FILL_DATE} filled from the daily-bar mirror for {filled} tickers")
...
pickle.dump({"cal": cal, "series": series}, open(f"{SCRATCH}/series7b.pkl", "wb"))

print(f"\nterminal session: {cal[-1]}  |  tickers aligned: {len(series)}")
have_last = sum(1 for s,(fi,cs,vs,ff) in series.items() if fi + len(cs) == len(cal))
print(f"series reaching {cal[-1]}: {have_last}")
