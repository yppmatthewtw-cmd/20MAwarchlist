...
VER = "R12.00"
OUTNAME = f"AI_Sector_watchlist_{VER}_claudeopus5high_{STAMP}.html"
WD = "一二三四五六日"
...
def table_main():
    ...
    for r in live:
        body.append(
            ...
            + f'<td class="nums mf {"pos" if r["mfd5"] > 0 else "neg"}">{fmt_m(r["mfd5"])}{prov_mark()}</td>'
    ...
    for r in live:
        ...
        body.append(
            ...
            + f'<td>{meter(r["score5"], "m5")}</td><td>{meter(r["score5r"], "m5")}</td>'
              ...
              f'<td class="nums mf {"pos" if r["mfd5"] > 0 else "neg"}">{fmt_m(r["mfd5"])}{prov_mark()}</td></tr>')
    return head, "".join(body)
...
def table_cat():
    ...
    for i, a in enumerate(agg, 1):
        body.append(
            ...
            + f'<td class="nums mf {"pos" if a["mfd"] > 0 else "neg"}">{fmt_m(a["mfd"])}{prov_mark()}</td>'
...
③ <b>逐日資金流向分</b>（每隻股票、每個交易日，與 Sub-Sector Watchlist R4.00 完全相同）：<br>
　<code>A 方向 = tanh((個股報酬 − 當日全巿場中位報酬) / 2%)</code>；
<code>B 量能 = log₂(當日<b>成交股數</b> / 前20日中位<b>成交股數</b>) ÷ 2</code>，截於 ±1；（R8 起用股數而非成交額：分子分母同用價格會令價格漂移混入參與度，實測 6.7% 嘅「股票×交易日」偏差超過 0.10。成交額仍負責籃子加權同淨額估算。）
...
⑦ <b>成交量</b>：本版視窗內 {M["n_tick"]} 隻美股／ADR 五日成交量與日內高低價齊備（日線鏡像未發佈嘅日子及 09-04 整日由 Yahoo 補齊），無「量?」格；
...
<div class="pghead"><b>象限背離</b><span>儀表板 RS 象限（{ASOF}）vs 本次實測資金流向（{md(DAYS[0])}–{md(DAYS[-1])}）· 相隔 {GAP} 個交易日</span></div>
<div class="qgrid">
...
② <b>價量數據</b>：natezone/market-tracker 日線 OHLCV 與 Yahoo Finance 日線合併（全部有日內高低價，可算 Chaikin 收位）；Nasdaq 快照重建序列只作交叉對照。計分視窗 <b>{DAYS[0]} → {DAYS[-1]}</b> 共 5 個交易日，
...
成分股全屬非美股嘅 {len(dead)} 個小群組列於總覽頂部紅框、唔參與排名。08-28 起先有數據嘅 ADR（TSM、ASML、ARM、BABA、BIDU 等）及 OTC ADR（SBGSY、TCEHY）本版全部由 Yahoo 日線提供完整歷史，無未計入成分股。<br>
④ <b>數據終點 {DAYS[-1]}（美東週{WD[datetime.date.fromisoformat(DAYS[-1]).weekday()]}收盤）</b>：建置時 {BUILD_TS}。
...
<b>全巿中位</b>取自平衡面板（五日同一批 {max(M["mkt_n"].values()):,} 隻）。<br>
...
計分方法與 Sub-Sector Watchlist R4.00 完全一致 · 未有以紅字標示變動（依指示）· 本表只係研究工具，唔係投資建議。
</div>"""
