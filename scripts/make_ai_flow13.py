...
src=open(f"{S}/sub12/flow12.py",encoding="utf-8").read()
def rep(old,new):
    global src
    assert old in src, old[:70]; src=src.replace(old,new,1)
rep('"""Sub-sector money-flow scoring — R12.00 engine (111 sub-sectors of the R2 heat-map workbook).',
...
rep('json.dump(out, open(os.environ.get("OUT_JSON", f"{SCR}/sub12/flow12.json"), "w"), ensure_ascii=False)',
...
open(f"{S}/ai13/flow13.py","w",encoding="utf-8").write(src); print("ai13/flow13.py written")
