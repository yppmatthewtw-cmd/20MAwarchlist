...
src=open(f"{S}/sub10/flow10.py",encoding="utf-8").read()
def rep(old,new):
    global src
...
rep('"""Sub-sector money-flow scoring — R10.00 engine (111 sub-sectors of the R2 heat-map workbook).',
    '"""AI money-flow scoring — R11.00 engine (41 AI 小群組 of the Dashboard R15.6 classification).\n\n'
    'Identical scoring math to the Sub-Sector watchlist R10.00 engine (Yahoo second source); only\n'
...
rep('json.dump(out, open(os.environ.get("OUT_JSON", f"{SCR}/sub10/flow10.json"), "w"), ensure_ascii=False)',
    ...
    'json.dump(out, open(os.environ.get("OUT_JSON", f"{SCR}/ai11/flow11.json"), "w"), ensure_ascii=False)')
...
open(f"{S}/ai11/flow11.py","w",encoding="utf-8").write(src); print("ai11/flow11.py written")
