...
src = open(f"{S}/ai8/verify8.py", encoding="utf-8").read()
def rep(a, b):
    global src
    assert src.count(a) == 1, (src.count(a), a[:90]); src = src.replace(a, b, 1)

rep('F = json.load(open(f"{SCR}/ai8/flow8.json"))', 'F = json.load(open(f"{SCR}/ai9/flow9.json"))')
...
open(f"{S}/ai9/verify9.py", "w", encoding="utf-8").write(src); print("ai9/verify9.py written")
