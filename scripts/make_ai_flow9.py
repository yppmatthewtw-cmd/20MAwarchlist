...
src=open(f"{S}/sub8/flow8.py",encoding="utf-8").read()
def rep(old,new):
    global src
...
rep('"""Sub-sector money-flow scoring — R8.00 engine (111 sub-sectors of the R2 heat-map workbook).',
    '"""AI money-flow scoring — R9.00 engine (41 AI 小群組 of the Dashboard R15.6 classification).\n\n'
    'Identical scoring math to the Sub-Sector watchlist R8.00 engine (Yahoo second source); only\n'
...
rep('json.dump(out, open(os.environ.get("OUT_JSON", f"{SCR}/sub8/flow8.json"), "w"), ensure_ascii=False)',
    ...
    'json.dump(out, open(os.environ.get("OUT_JSON", f"{SCR}/ai9/flow9.json"), "w"), ensure_ascii=False)')
...
open(f"{S}/ai9/flow9.py","w",encoding="utf-8").write(src); print("ai9/flow9.py written")
