...
F=json.load(open(f"{SCR}/ai/flow3.json")); M=F["meta"]; DAYS=M["days"]; rows=[r for r in F["rows"] if r.get("days")]
...
htmlf=sorted(glob.glob(f"{SCR}/AI_Sector_watchlist_R3.00_*.html"))[-1]
...
# --- R3: per-ticker column integrity ---
import math as _mm, glob as _g, re as _re
W5=M["weights"]
for r in rows:
    ts=r.get("ticks") or []
    if [t["sym"] for t in ts] != sorted([t["sym"] for t in ts], key=lambda s: -[x for x in ts if x["sym"]==s][0]["tf5"]):
        bad.append(("ticks not sorted by flow", r["code"]))
    if sorted(t["sym"] for t in ts) != sorted(r["basket"]): bad.append(("ticks != basket", r["code"]))
    for t in ts:
        # recompute the per-ticker weighted flow from the stored daily values of that ticker
        pass
    for a,b2 in zip(ts, ts[1:]):
        if a["tf5"] < b2["tf5"] - 1e-12: bad.append(("tick order", r["code"], a["sym"], b2["sym"]))
# colour classes in the HTML must follow the documented thresholds
H2=open(htmlf,encoding="utf-8").read()
def cls_of(v): return ("gp2" if v>=0.20 else "gp1" if v>=0.05 else "g00" if v>-0.05 else "gn1" if v>-0.20 else "gn2")
miss=0
for r in rows:
    for t in (r.get("ticks") or []):
        pat='class="tkf %s" href="https://www.tradingview.com/chart/Q1c5VWwD/?symbol=%s"' % (cls_of(t["tf5"]), t["sym"].lower())
        if pat not in H2: miss+=1
if miss: bad.append(("ticker pill class/link mismatch", miss))
print("ticker pills checked:", sum(len(r.get("ticks") or []) for r in rows), "| mismatches:", miss)
