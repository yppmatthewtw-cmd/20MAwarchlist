...
F=json.load(open(f"{SCR}/sub/flow2.json")); M=F["meta"]; DAYS=M["days"]; rows=[r for r in F["rows"] if r.get("days")]
S8=pickle.load(open(f"{SCR}/series9.pkl","rb")); CAL=S8["cal"]; SER=S8["series"]
bad=[]
# 1) structural
if len(F["rows"])!=111: bad.append(("row count", len(F["rows"])))
if len(rows)!=111: bad.append(("scored rows", len(rows)))
subs=json.load(open(f"{SCR}/sub/subsectors.json"))
if [r["n"] for r in sorted(rows,key=lambda x:x["n"])]!=[s["n"] for s in subs]: bad.append(("sub-sector ids mismatch",))
...
if [r["rank"] for r in sorted(rows,key=lambda r:-r["z5"])]!=list(range(1,112)): bad.append(("rank order",))
for r in rows:
...
htmlf=sorted(glob.glob(f"{SCR}/SubSector_flow_watchlist_R2.00_*.html"))[-1]
H=open(htmlf,encoding="utf-8").read()
for r in rows[:40]+rows[-20:]:
...
print("basket size: min %d, median %.0f, max %d | n<=2: %d" % (min(r["n_basket"] for r in rows),
      statistics.median(r["n_basket"] for r in rows), max(r["n_basket"] for r in rows),
      sum(1 for r in rows if r["n_basket"]<=2)))
import math as _m
for r in rows:
    n=r["n_basket"]
    if abs(r["shrink"]-_m.sqrt(n/(n+2.0)))>1e-9: bad.append(("shrink",r["n"]))
    if abs(r["z5r"]-r["z5"]*r["shrink"])>1e-9: bad.append(("z5r",r["n"]))
    if r["dv5"] and abs(r["intensity"]-r["mfd5"]/r["dv5"]*100)>1e-6: bad.append(("intensity",r["n"]))
    if abs(r["breadth5"]-statistics.mean(r["days"][d]["breadth"] for d in DAYS))>1e-9: bad.append(("breadth5",r["n"]))
