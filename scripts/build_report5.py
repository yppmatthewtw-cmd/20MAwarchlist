...
NEWS = json.load(open(f"{SCRATCH}/news5.json")) if os.path.exists(f"{SCRATCH}/news5.json") else {}
...
OUTNAME = f"20MA_uptrend_watchlistGit_R5.00_claudeopus5max_{STAMP}.html"
...
c = M["counts"]
UNIVERSE_LINE = (f'全美上市普通股掃描 — 快照涵蓋 {c["total"]:,} 隻 · 存續至 08-28 有報價 {c["current"]:,} 隻 · '
...
html_doc = f"""<title>20MA Uptrend Watchlist R5</title>
<style>{css}</style>
<div class="wrap">
<h1>20MA Uptrend Watchlist R5（大／中／小型股分層 × VCP × 底部確定性）</h1>
<div class="sub">數據至 <b>{M["last_date"]}</b> 收盤 · 全美掃描 {c["liq"]:,} 隻合資格 · 12 個分層榜（4 個時間框 × 大/中/小型股）＋總表 · 🔥 熱炒 news-driven 催化劑獨立成欄並喺每頁頂部標示</div>
{rules_html}
<nav class="nav">
...
(function() {{
  ...
  function show() {{
    ...
    tierrow.hidden = (g === '1');
    syncSort();
    try {{ localStorage.setItem('r5nav', g + '|' + t); }} catch (e) {{}}
  }}
  var sortState = {{}};
  ...
  try {{
    var sv = localStorage.getItem('r5nav');
    if (sv) {{
      var parts = sv.split('|');
