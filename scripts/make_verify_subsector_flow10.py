...
src = open(f"{S}/sub9/verify9.py", encoding="utf-8").read()
def rep(a, b):
    global src
    assert src.count(a) == 1, (src.count(a), a[:90]); src = src.replace(a, b, 1)

rep('F = json.load(open(f"{SCR}/sub9/flow9.json"))', 'F = json.load(open(f"{SCR}/sub10/flow10.json"))')
...
open(f"{S}/sub10/verify10.py", "w", encoding="utf-8").write(src); print("sub10/verify10.py written")
