#!/usr/bin/env python3
"""AI money-flow scoring — R4.00 engine (41 AI 小群組 of the Dashboard R15.6 classification).

Identical scoring math to the Sub-Sector watchlist R3.00 engine; only the baskets differ.
Per the brief, baskets hold US-listed shares and US ADRs only — members carrying a
foreign-exchange suffix are excluded and reported, not substituted.

Carries the same critical-review fixes as Sub-Sector R3.00 (see below).
...
seq = [d for d in CAL if d in set(full)]
...
def tick_flow(sym):
    ...
    out = {"sym": sym, "src": "real" if real else "snap", "med_dv": med_dv,
           "nobase": nobase, "days": {}}
    for day in DAYS:
        prev = LAST[LAST.index(day) - 1]
...
out = {"meta": {"days": DAYS, "base": [BASE[0], BASE[-1]], "n_base": len(BASE),
                ...
                "terminal_note": M10.get("note_0904", ""),
                "source_note": "natezone/market-tracker 真實日線 OHLCV 為主，其餘以快照序列（僅收盤，成交量有則用）補足"},
...
json.dump(out, open(os.environ.get("OUT_JSON", f"{SCR}/ai4/flow4.json"), "w"), ensure_ascii=False)
print("\nTOP 12 inflow:")
for r in live[:12]:
