...
merged = dict(json.load(open(f"{SCRATCH}/news5.json")))   # 221 names carried forward
for e in merged.values():
    e.setdefault("src", "R5")
carried = len(merged)

added = 0
files = sorted(glob.glob(f"{SCRATCH}/news6_*.json"))
for f in files:
    try:
        d = json.load(open(f))
    ...
    for sym, e in d.items():
        ...
        added += 1
print(f"carried from R5: {carried} | R6 batch files: {len(files)} entries {added} | total {len(merged)}")
json.dump(merged, open(f"{SCRATCH}/news6.json", "w"), ensure_ascii=False)
