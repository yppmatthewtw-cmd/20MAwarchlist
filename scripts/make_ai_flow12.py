...
src=open(f"{S}/sub11/flow11.py",encoding="utf-8").read()
def rep(old,new):
    global src
    assert old in src, old[:70]; src=src.replace(old,new,1)
rep('"""Sub-sector money-flow scoring — R11.00 engine (111 sub-sectors of the R2 heat-map workbook).',
    '"""AI money-flow scoring — R12.00 engine (41 AI 小群組 of the Dashboard R15.6 classification).\n\n'
    'Identical scoring math to the Sub-Sector watchlist R11.00 engine (Yahoo second source); only\n'
...
rep('json.dump(out, open(os.environ.get("OUT_JSON", f"{SCR}/sub11/flow11.json"), "w"), ensure_ascii=False)',
    'out["meta"]["asof_dash"] = AI["asof_dash"]\nout["meta"]["nonus_excluded"] = sorted({m for s in subs for m in s["nonus"]})\n'
    'json.dump(out, open(os.environ.get("OUT_JSON", f"{SCR}/ai12/flow12.json"), "w"), ensure_ascii=False)')
...
# the dashboard's AI-group panel carries its own as-of date; record the real session distance
rep('out["meta"]["asof_dash"] = AI["asof_dash"]',
    'out["meta"]["asof_dash"] = AI["asof_dash"]\n'
    'out["meta"]["asof_gap_sessions"] = gap_sessions(AI["asof_dash"], DAYS[-1])')
open(f"{S}/ai12/flow12.py","w",encoding="utf-8").write(src); print("ai12/flow12.py written")
