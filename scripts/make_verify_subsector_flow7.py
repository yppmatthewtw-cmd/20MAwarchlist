...
src = open(f"{S}/sub6/verify6.py", encoding="utf-8").read()
def rep(a, b):
    global src
    assert src.count(a) == 1, (src.count(a), a[:80]); src = src.replace(a, b, 1)

rep('F = json.load(open(f"{SCR}/sub6/flow6.json"))', 'F = json.load(open(f"{SCR}/sub7/flow7.json"))')
...
open(f"{S}/sub7/verify7.py", "w", encoding="utf-8").write(src); print("sub7/verify7.py written")
