...
F = json.load(open(f"{SCRATCH}/sub/flow2.json"))
...
OUTNAME = f"SubSector_flow_watchlist_R2.00_claudeopus5high_{STAMP}.html"
WD = "一二三四五六日"
...
upd = f"""
<div class="upd"><h3>本版更新（R2.00 vs R1.01）</h3><ul>
<li><b>數據推進一個交易日</b>：計分視窗由 {BDAYS[0]} → {BDAYS[-1]} 改為 <b>{DAYS[0]} → {DAYS[-1]}</b>
（新增 {esc("、".join(NEWD))}，移出 {esc("、".join(GONED))}）—— 順帶將 R1.01 視窗內嘅 08-26 估算日移走。
{DAYS[-1]} 收盤取自 09-03 盤中快照嘅 <code>price − price_change</code>（即官方前收），已與日線鏡像 1,498 隻交叉核對，
中位偏差 0.0000%。<b>9月3日未納入</b>：日線鏡像當日只出到 1,003/1,503 隻，亦未有全宇宙快照。</li>
<li><b>成交量缺口處理</b>：09-03 快照嘅成交量係當日 partial，故 {DAYS[-1]} 只有日線鏡像覆蓋嘅股票有真實成交量；
其餘股票該日<b>量能項 B 設為中性 0</b>（唔用估算量冒充），格內標「量?」—— 本版 {n_nv} 個子板塊受影響。</li>
<li><b>新增「穩健分」</b>：原始分對樣本細嘅子板塊噪音大（鈾與核燃料只得 1 隻、11 個子板塊 ≤2 隻）。
穩健分 = 5日 z ×√(n/(n+2)) 後取百分位，樣本愈細愈向中性收斂；兩個分數都可排序，可對照睇。</li>
<li><b>新增「資金強度」</b>：淨額估算 ÷ 同期成交額（%），去除規模效應 —— 大板塊淨額天然大，
呢欄先睇得出「每一蚊成交額入面有幾多係買方主導」。</li>
<li><b>新增「可信度」與「廣度」</b>：可信度按樣本數（≤2 低／3 中／≥4 高）；廣度＝籃子內超額報酬為正嘅比例，
用嚟分辨「全組齊升」定「一兩隻股拉起」。</li>
<li><b>市況逆轉</b>：{DAYS[-1]} 全巿中位 <b>{mkt[DAYS[-1]]*100:+.2f}%</b>，扭轉之前連續四個交易日下跌，
{len(moved)} 個子板塊名次有變。升幅最大 {esc("、".join(up3)) or "無"}；跌幅最大 {esc("、".join(dn3)) or "無"}。</li>
</ul></div>"""
...
{DAYS[-1]} 只有日線鏡像覆蓋嘅股票有真實成交量，其餘股票該日量能項設為中性、格內標「量?」，唔以估算量冒充。<br>
