...
grew = sorted(((c, BROW[c]["n_basket"], cur[c]["n_basket"]) for c in cur if c in BROW and BROW[c]["n_basket"] != cur[c]["n_basket"]),
...
② <b>數據終點</b>：本表以 <b>{DAYS[-1]}（美東週{WD[datetime.date.fromisoformat(DAYS[-1]).weekday()]}）收盤</b>為最後一日 ——
...
本表唔以盤中價或成交量單邊數據冒充收盤。</span><br>
③ <b>08-31 快照缺口</b>：該日無全宇宙快照，非日線鏡像覆蓋嘅股票收盤以 09-01 快照 price − price_change（官方前收）補回、屬精確值；
成交量則無來源，本版視為未知（量能項中性、標「量?」）；只有 13 隻無法還原官方收盤嘅以前後內插，喺「樣本」欄標「內插」。<br>
