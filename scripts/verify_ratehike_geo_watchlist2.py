...
FN  = 'reports/RateHike_Geopolitical_3Day_Watchlist_R2.00_claudeopus5high_09.18_0900.xlsx'
...
for tk in MISS:                       # dropped for want of a base-session bar, not carried stale
    if tk in WTK or tk in ATK: bad(f"{tk} has no {'2026-09-17'} bar but is still listed")
    if tk in seen:             bad(f"{tk} has no base-session bar but is in the index")
...
univ = (set(WTK) | set(ATK) | {t for s in SEC['rows'] for t in s['tick'].split(',')}) - set(MISS)
...
# 5b. Scorecard: every move column is read from the flow data, so check it against the source
CARD_S = '判斷記分卡 Scorecard'
sc = V[CARD_S]
RANK_NOW = {x['zh']: i for i, x in enumerate(sorted(SEC['rows'], key=lambda y: -y['s5']), 1)}
SECBY = {x['zh']: x for x in SEC['rows']}
r, ncard = 9, 0
while sc.cell(r, 2).value:
    zh = sc.cell(r, 2).value
    if zh not in SECBY:
        bad(f"scorecard row {r}: unknown sub-sector {zh}")
    else:
        if not close(sc.cell(r, 4).value, SECBY[zh]['r16']):  bad(f"scorecard {zh} 9/17 ret")
        if not close(sc.cell(r, 5).value, SECBY[zh]['bd16']): bad(f"scorecard {zh} breadth")
        if sc.cell(r, 6).value != SEC['prev_rank'].get(zh):   bad(f"scorecard {zh} prev rank")
        if sc.cell(r, 7).value != RANK_NOW[zh]:               bad(f"scorecard {zh} new rank")
        if sc.cell(r, 8).value != sc.cell(r, 6).value - sc.cell(r, 7).value: bad(f"scorecard {zh} delta")
    ncard += 1; r += 1
print(f"scorecard rows verified: {ncard}")
if ncard < 20: bad(f"scorecard has only {ncard} rows")
...
if cnt != 9: bad(f"scenario row count {cnt} != 9")
if not close(s.cell(r,6).value, sum(s.cell(x,6).value for x in range(hdr+1,hdr+7))/6): bad("scenario benefit avg")
if not close(s.cell(r+1,6).value, sum(s.cell(x,6).value for x in range(hdr+8,hdr+10))/2): bad("scenario avoid avg")
