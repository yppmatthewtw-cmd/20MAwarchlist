#!/usr/bin/env python3
"""R6 screener: R3/R4 rules (unchanged) + market-cap tiering.
...
d = pickle.load(open(f"{SCRATCH}/series6.pkl", "rb"))
...
# ---- names, GICS sector/industry and index membership from the price mirror ----
meta = {}
with open(f"{NZ}/data/COMBINED/latest_metrics.csv", newline="") as f:
    for row in csv.DictReader(f):
        s = row["ticker"].strip()
        meta[s] = {"name": (row.get("company_name") or s).strip(),
                   "sector": (row.get("sector") or "—").strip() or "—",
                   "industry": (row.get("industry") or "—").strip() or "—",
                   "idx": "", "mcap": 0.0, "sp500": False}

IDX_LABEL = {"SP500": "S&P500", "SP400": "S&P400", "SP600": "S&P600"}
for ix in ("SP500", "SP400", "SP600"):
    p = f"{NZ}/data/{ix}/latest_metrics.csv"
    if not os.path.exists(p):
        continue
    with open(p, newline="") as f:
        for row in csv.DictReader(f):
            s = row["ticker"].strip()
            m = meta.setdefault(s, {"name": s, "sector": "—", "industry": "—",
                                    "idx": "", "mcap": 0.0, "sp500": False})
            m["idx"] = IDX_LABEL[ix]
            if ix == "SP500":
                m["sp500"] = True

# ---- market cap: shares implied by the Nasdaq snapshot, revalued at the latest close ----
shares = {}
...
for row in csv.DictReader(io.StringIO(blob.lstrip("﻿"))):
    s = row["symbol"].strip()
    try:
        mc = float(row["market_cap"] or 0); px = float(row["price"] or 0)
    except (ValueError, TypeError):
        continue
    if mc > 0 and px > 0:
        shares[s] = mc / px
mc_revalued = mc_fallback = 0
for s, m in meta.items():
    sh = shares.get(s) or shares.get(norm(s))
    ser = SER.get(s)
    if sh and ser:
        m["mcap"] = sh * ser[1][-1]
        mc_revalued += 1
        ...
        m["mcap"] = 0.0
        mc_fallback += 1
print(f"metadata: {len(meta)} rows; market cap revalued at {LAST_DATE} for {mc_revalued}")

exch = {}
for ex in ("NASDAQ", "NYSE", "AMEX"):
    p = f"{IRA}/symbols/{ex}.csv"
    if not os.path.exists(p):
        continue
...
ZH_SECTOR = {  # GICS
    ...
    "Industrials": "工業", "Energy": "能源", "Materials": "原材料",
...
TIERS = [("a", "big", "大型股", "S&P 500"),
         ("b", "mid", "中型股", "S&P 400"),
         ("c", "small", "小型股", "S&P 600")]
IDX_TIER = {"S&P500": "a", "S&P400": "b", "S&P600": "c"}

def cap_tier(idx_label):
    """Tier by S&P index membership - the standard large/mid/small definition and
    the natural split for an S&P 1500 universe. Anything outside the three
    indices returns "x" and is left out of the cap-tier pages."""
    return IDX_TIER.get(idx_label, "x")
...
def row(sym, q):
    ...
    return {
        ...
        "gsec": m.get("sector", "—"), "gsub": m.get("industry", "—"),
        "sp500": m.get("sp500", False), "idx": m.get("idx", ""),
        "mcap": m.get("mcap", 0.0), "tier": cap_tier(m.get("idx", "")),
...
out = {"meta": {"last_date": LAST_DATE, "cal_first": CAL[0], "cal_last": CAL[-1],
                ...
                "tiers": [{"key": k, "en": en, "zh": zh, "index": idxname}
                          for k, en, zh, idxname in TIERS]},
       "pages": {}}
...
print(f"不屬 S&P 1500 三大指數（已排除出分層頁）: {len(unknown_syms)} 隻 {unknown_syms[:12]} | 各時間框 {unknown}")
for p, (L, W, label) in PAGES.items():
    for key, en, zh, idxname in TIERS:
    ...
    tot = sum(out["pages"][f"{p}{k}"]["qualified"] for k, _, _, _ in TIERS) + unknown[str(p)]
    print(f"  守恆 P{p}: 三層 + 未知市值 {unknown[str(p)]} = {tot}")

PAGE_IDS = [f"{p}{k}" for p in PAGES for k, _, _, _ in TIERS]
listed = {}
for pid in PAGE_IDS:
