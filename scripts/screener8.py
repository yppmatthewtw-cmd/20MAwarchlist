#!/usr/bin/env python3
"""R7 screener: R3/R4 rules (unchanged) + market-cap tiering.

Each timeframe splits into three cap tiers, each ranked independently by
綜合 = 0.5 x VCP + 0.5 x certainty and cut at top 50:
  a 大型股 big cap    market cap >= $10B
  b 中型股 mid cap    $2B <= market cap < $10B
  c 小型股 small cap  market cap < $2B (includes micro caps that pass liquidity)

Original R3 header follows.
R3 screener: R2 qualification rules (unchanged) + 7 bottom-certainty metrics.

確定性 (certainty, 0-100) components and weights:
  s_break 25%  breakout of the intermediate high between the last two bottoms
               (1.0 if the post-bottom high exceeds it; otherwise 0.6 x progress)
  s_retr  10%  current recovery of the decline leg into the last bottom, capped at 100%
  s_time  15%  trading days the last bottom has held, full marks at 15 obs;
               x0.25 if any later close undercut the bottom
  s_dv    15%  down-day volume / up-day volume over the last 15 obs (percentile, lower better)
  s_contr 10%  depth of last decline leg / depth of first leg in the 45-obs window
               (percentile, lower better)
  s_rs    10%  21-obs return minus the eligible-universe median (percentile)
  s_ma    15%  0.4 x (close > MA20) + 0.3 x (MA20 > MA50) + 0.3 x (MA50 rising over 10 obs)

Pages 2-5 rank by 綜合 = 0.5 x VCP + 0.5 x 確定性 (top 50 by unrounded value).
Page 1 爆發潛力 = 0.4 x VCP + 0.4 x 確定性 + 0.2 x (qualifying frames / 4 x 100).
"""
import csv, io, json, os, pickle, statistics, subprocess

SCRATCH = "/tmp/claude-0/-home-user-20MAwarchlist/0f749aae-85b5-584c-9175-237303814dd9/scratchpad"
ZREPO = "/home/user/zyhe16/top-us-stock-tickers"
MC = "/home/user/klaywang24/market-chronicle"
IRA = "/home/user/irachex/open-stock-data"

d = pickle.load(open(os.environ.get("SERIES", f"{SCRATCH}/series8.pkl"), "rb"))
CAL, SER = d["cal"], d["series"]
META8 = d.get("meta8", {})
EST = META8.get("estimated", {})          # sym -> {date: real|official|interp|vol_scaled}
DEAL_FILE = os.environ.get("DEALS", f"{SCRATCH}/r8/deal_pinned.json")
DEALS = json.load(open(DEAL_FILE)) if os.path.exists(DEAL_FILE) else {}
FLAT15 = float(os.environ.get("FLAT15", "2.0"))   # data-only pinned-tape rule (% range of last 15 closes); 0 = off
import re
def is_fund(name, industry):
    """Closed-end funds, term/municipal trusts, royalty trusts: NAV-anchored instruments whose
    volatility contraction is structural, not a base. REITs / banks / MLPs / BDCs are kept."""
    if industry.startswith("Trusts Except"): return True
    blob = f"{name} {industry}"
    if re.search(r"Real Estate|Realty|Propert|Bank|REIT|Mortgage", blob, re.I): return False
    return bool(re.search(r"\bFund\b|Royalty Trust|Income Trust|Target Term|Municipal|Floating Rate|Closed[- ]End|Beneficial Interest|Opportunities Corp", name))
def issuer_key(name):
    n = re.sub(r"\b(Class [A-C]|Common Stock|Common Shares|Ordinary Shares|Series [A-Z]|\(.*?\))\b", "", name, flags=re.I)
    return re.sub(r"[^a-z0-9]", "", n.lower())
LAST_DATE = CAL[-1]

def norm(sym):
    return sym.replace("/", ".").strip().upper()

meta = {}
blob = subprocess.run(["git", "-C", ZREPO, "show", "origin/main:data/v2/tickers.csv"],
                      capture_output=True, text=True).stdout
for row in csv.DictReader(io.StringIO(blob.lstrip("﻿"))):
    s = row["symbol"].strip()
    meta[s] = {
        "name": row["name"].split(" Common Stock")[0].split(" Ordinary Shares")[0].strip().rstrip(","),
        "sector": row["sector"].strip() or "—",
        "industry": row["industry"].strip() or "—",
        "country": row["country"].strip(),
        "sp500": row["is_sp500"].strip() == "True",
        "mcap": float(row["market_cap"] or 0),
    }

gics = {}
mcj = json.load(open(f"{MC}/data/sp500_constituents.json"))
for r in mcj["rows"]:
    gics[norm(r["ticker"])] = {"gsec": r["sector"], "gsub": r["sub"]}

exch = {}
for ex in ("NASDAQ", "NYSE", "AMEX"):
    with open(f"{IRA}/symbols/{ex}.csv", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            exch[norm(row["code"])] = ex

ZH_SECTOR = {
    "Technology": "科技", "Consumer Discretionary": "非必需消費", "Health Care": "醫療保健",
    "Finance": "金融", "Industrials": "工業", "Consumer Staples": "必需消費",
    "Energy": "能源", "Real Estate": "房地產", "Utilities": "公用事業",
    "Basic Materials": "原材料", "Telecommunications": "電訊", "Miscellaneous": "其他", "—": "—",
}

def sma_series(cs, L):
    out = [None] * len(cs)
    run = 0.0
    for i, c in enumerate(cs):
        run += c
        if i >= L: run -= cs[i - L]
        if i >= L - 1: out[i] = run / L
    return out

def find_bottoms(cs):
    n = len(cs); raw = []
    for i in range(3, n - 3):
        w = cs[i - 3:i + 4]
        if cs[i] == min(w) and cs[i - 3] > cs[i] and cs[i + 3] > cs[i]:
            raw.append((i, cs[i]))
    dedup = []
    for i, c in raw:
        if dedup and i - dedup[-1][0] <= 3:
            if c < dedup[-1][1]: dedup[-1] = (i, c)
        else:
            dedup.append((i, c))
    return dedup

def higher_lows(bots, n, look=45, recent=25):
    inw = [(i, c) for i, c in bots if i >= n - look]
    if len(inw) < 2: return None
    for a, b in zip(inw, inw[1:]):
        if b[1] <= a[1]: return None
    if inw[-1][0] < n - recent: return None
    return inw

def ma_uptrend(ma, W):
    n = len(ma)
    if n < W + 1 or ma[-1] is None or ma[-1 - W] is None: return None
    if not ma[-1] > ma[-1 - W]: return None
    if not (ma[-1] > ma[-2] > ma[-3]): return None
    diffs = [1 if ma[-k] > ma[-k - 1] else 0 for k in range(1, W + 1)]
    if sum(diffs) / W < 0.70: return None
    return (ma[-1] / ma[-1 - W] - 1) * 100

# ---------------- eligibility + VCP raw components (identical to R2) ----------------
PAGES = {2: (10, 5, "1星期"), 3: (20, 10, "2星期"), 4: (20, 21, "1個月"), 5: (20, 42, "2個月")}
elig = {}
stats_counts = {"total": len(SER), "current": 0, "hist": 0, "price": 0, "liq": 0}
for sym, (fi, cs, vs, ff) in SER.items():
    if fi + len(cs) != len(CAL):
        continue
    stats_counts["current"] += 1
    if len(cs) < 90: continue
    stats_counts["hist"] += 1
    if cs[-1] < 2.0: continue
    stats_counts["price"] += 1
    dv = [c * v for c, v in zip(cs[-20:], vs[-20:])]
    if statistics.median(dv) < 1_000_000: continue
    stats_counts["liq"] += 1

    rets = [cs[i] / cs[i - 1] - 1 for i in range(1, len(cs))]
    s_rec = statistics.pstdev(rets[-10:])
    s_pri = statistics.pstdev(rets[-40:-10])
    if s_pri <= 1e-9: continue  # degenerate flat series; never fires on current data
    cr = s_rec / s_pri
    t10 = (max(cs[-10:]) - min(cs[-10:])) / cs[-1]
    v_rec = sum(vs[-10:]) / 10
    v_pri = sum(vs[-40:-10]) / 30
    vr = v_rec / v_pri if v_pri > 0 else 1.0
    blocks = [cs[-45:-30], cs[-30:-15], cs[-15:]]
    rng = [(max(b) - min(b)) / (sum(b) / len(b)) for b in blocks]
    pc = rng[2] / rng[0] if rng[0] > 1e-9 else 1.0
    elig[sym] = {"cr": cr, "t10": t10, "vr": vr, "pc": pc}

def pct_ranks(vals):
    order = sorted(range(len(vals)), key=lambda i: vals[i])
    r = [0.0] * len(vals)
    for rank, i in enumerate(order):
        r[i] = rank / (len(vals) - 1) if len(vals) > 1 else 0.5
    return r

syms = list(elig)
for comp in ("cr", "t10", "vr", "pc"):
    pr = pct_ranks([elig[s][comp] for s in syms])
    for s, p in zip(syms, pr): elig[s]["pr_" + comp] = p
for s in syms:
    e = elig[s]
    e["vcp_raw"] = 100 * (0.35 * (1 - e["pr_cr"]) + 0.25 * (1 - e["pr_t10"])
                          + 0.20 * (1 - e["pr_vr"]) + 0.20 * (1 - e["pr_pc"]))
    e["vcp"] = round(e["vcp_raw"], 1)

# ---------------- universe-wide certainty inputs ----------------
mkt_rets = {}
for s in syms:
    cs = SER[s][1]
    mkt_rets[s] = cs[-1] / cs[-22] - 1
mkt_med = statistics.median(mkt_rets.values())

univ = {}
for s in syms:
    fi, cs, vs, ff = SER[s]
    # down-day vs up-day volume over last 15 obs
    dn, up = [], []
    for i in range(len(cs) - 15, len(cs)):
        if cs[i] > cs[i - 1]: up.append(vs[i])
        elif cs[i] < cs[i - 1]: dn.append(vs[i])
    dv_ratio = (sum(dn) / len(dn)) / (sum(up) / len(up)) if dn and up and sum(up) > 0 else 1.0
    # MA structure
    ma20 = sma_series(cs, 20); ma50 = sma_series(cs, 50)
    f_c20 = cs[-1] > ma20[-1]
    f_2050 = ma20[-1] > ma50[-1] if ma50[-1] else False
    f_50up = (ma50[-1] > ma50[-11]) if (ma50[-1] and ma50[-11]) else False
    s_ma = 0.4 * f_c20 + 0.3 * f_2050 + 0.3 * f_50up
    univ[s] = {"rs21": mkt_rets[s] - mkt_med, "dv_ratio": dv_ratio,
               "s_ma": s_ma, "ma_flags": [bool(f_c20), bool(f_2050), bool(f_50up)]}

pr_rs = pct_ranks([univ[s]["rs21"] for s in syms])
pr_dv = pct_ranks([univ[s]["dv_ratio"] for s in syms])
for s, a, b in zip(syms, pr_rs, pr_dv):
    univ[s]["s_rs"] = a          # higher RS -> higher percentile -> better
    univ[s]["s_dv"] = 1 - b      # lower down/up volume ratio -> better

# ---------------- structure metrics for tickers with a higher-lows sequence ----------------
struct = {}
contr_vals = {}
GATED = []
for s in syms:
    fi, cs, vs, ff = SER[s]
    n = len(cs)
    bots = find_bottoms(cs)
    hl = higher_lows(bots, n)
    if hl is None: continue
    bP, pP = hl[-2]
    bL, pL = hl[-1]
    if cs[-1] < pL:                      # R8: 現價已跌穿最後底 -> 一底高於一底已失效
        GATED.append(s); continue
    H_mid = max(cs[bP:bL])                      # intermediate high between last two bottoms
    post_high = max(cs[bL + 1:])                # bottoms exist only at i <= n-4
    C = cs[-1]
    broke = post_high > H_mid
    progress = (C - pL) / (H_mid - pL) if H_mid > pL else 1.0
    s_break = 1.0 if broke else 0.6 * max(0.0, min(1.0, progress))
    retrace = progress
    s_retr = max(0.0, min(1.0, retrace))
    d_held = (n - 1) - bL
    undercut = min(cs[bL + 1:]) < pL * 0.999
    s_time = min(1.0, d_held / 15) * (0.25 if undercut else 1.0)
    # depth contraction across the window's legs: first leg's high = 10 obs before first bottom
    depths = []
    prev_i = None
    for k, (bi, bp) in enumerate(hl):
        if k == 0:
            hi = max(cs[max(0, bi - 10):bi]) if bi > 0 else bp
        else:
            hi = max(cs[prev_i:bi])
        if hi > bp:
            depths.append((hi - bp) / hi)
        prev_i = bi
    contr = depths[-1] / depths[0] if len(depths) >= 2 and depths[0] > 1e-4 else 1.0
    contr_vals[s] = contr
    struct[s] = {"bP": bP, "pP": pP, "bL": bL, "pL": pL, "H_mid": H_mid,
                 "post_high": post_high, "broke": broke, "retrace": retrace,
                 "d_held": d_held, "undercut": undercut, "contr": contr,
                 "s_break": s_break, "s_retr": s_retr, "s_time": s_time,
                 "hl": hl, "bots": bots}

c_syms = list(contr_vals)
pr_contr = pct_ranks([contr_vals[s] for s in c_syms])
for s, p in zip(c_syms, pr_contr):
    struct[s]["s_contr"] = 1 - p                # lower contraction ratio -> better

for s in struct:
    st = struct[s]; u = univ[s]
    cert_raw = 100 * (0.25 * st["s_break"] + 0.10 * st["s_retr"] + 0.15 * st["s_time"]
                      + 0.15 * u["s_dv"] + 0.10 * st["s_contr"]
                      + 0.10 * u["s_rs"] + 0.15 * u["s_ma"])
    st["cert_raw"] = cert_raw
    st["cert"] = round(cert_raw, 1)
    st["combo_raw"] = 0.5 * elig[s]["vcp_raw"] + 0.5 * cert_raw
    st["combo"] = round(st["combo_raw"], 1)

# ---------------- per-page qualification (identical rules to R2) ----------------
qual = {p: {} for p in PAGES}
for s in struct:
    fi, cs, vs, ff = SER[s]
    ma10 = sma_series(cs, 10); ma20 = sma_series(cs, 20)
    for p, (L, W, _) in PAGES.items():
        ma = ma10 if L == 10 else ma20
        slope = ma_uptrend(ma, W)
        if slope is None: continue
        q = {"slope": slope, "ma": ma[-1], "L": L, "W": W}
        if p == 2:
            # does the 10MA rise survive neutralising the largest single up-day of the last 10 bars?
            rets = [(cs[i] / cs[i - 1] - 1, i) for i in range(len(cs) - 10, len(cs))]
            rmax, imax = max(rets)
            if rmax > 0.06:
                adj = cs[:imax] + [c / (1 + rmax) for c in cs[imax:]]
                q["single_day"] = ma_uptrend(sma_series(adj, L), W) is None
                q["single_day_pct"] = round(rmax * 100, 1)
        qual[p][s] = q

# share-class dedupe (e.g. NWS/NWSA): one issuer, one slot -- keep the more liquid class
by_issuer = {}
for s in list(struct):
    m = meta.get(s) or meta.get(norm(s)) or {}
    by_issuer.setdefault(issuer_key(m.get("name", s)), []).append(s)
CLASS_DUPES = []
for key, group in by_issuer.items():
    if len(group) < 2 or not key: continue
    def liq(s):
        fi, cs, vs, ff = SER[s]
        return statistics.median(c * v for c, v in zip(cs[-20:], vs[-20:]))
    group.sort(key=liq, reverse=True)
    for s in group[1:]:
        CLASS_DUPES.append({"dropped": s, "kept": group[0]})
        struct.pop(s, None)
        for p in PAGES: qual[p].pop(s, None)
print("share-class duplicates dropped:", CLASS_DUPES)
hits = {s: sum(1 for p in PAGES if s in qual[p]) for s in struct}

TIERS = [("a", "big", "大型股", 10e9, float("inf")),
         ("b", "mid", "中型股", 2e9, 10e9),
         ("c", "small", "小型股", 0.0, 2e9)]

def cap_tier(mcap):
    """Market-cap tier. Instruments with no cap in the Nasdaq feed (preferred /
    depositary shares, SPACs, closed-end funds) return "x" and are left out of
    the cap-tier pages rather than silently counted as small caps."""
    if not mcap or mcap <= 0:
        return "x"
    for key, _, _, lo, hi in TIERS:
        if lo <= mcap < hi:
            return key
    return "x"

def row(sym, q):
    fi, cs, vs, ff = SER[sym]
    m = meta.get(sym) or meta.get(norm(sym)) or {}
    g = gics.get(norm(sym), {})
    st = struct[sym]; u = univ[sym]
    n = len(cs)
    off = n - 60
    spark_b = [(i - off, round(c, 4)) for i, c in st["bots"] if i >= off]
    return {
        "sym": sym, "name": m.get("name", sym), "exch": exch.get(norm(sym), "—"),
        "sector": m.get("sector", "—"), "sector_zh": ZH_SECTOR.get(m.get("sector", "—"), m.get("sector", "—")),
        "industry": m.get("industry", "—"),
        "gsec": g.get("gsec"), "gsub": g.get("gsub"), "sp500": m.get("sp500", False),
        "mcap": m.get("mcap", 0.0), "tier": ("f" if is_fund(m.get("name", sym), m.get("industry", "")) else cap_tier(m.get("mcap", 0.0))),
        "close": round(cs[-1], 2), "ma": round(q["ma"], 2), "below_ma": cs[-1] < q["ma"],
        "ma_turn": cs[-1] < cs[-q["L"]],          # MA turns down on one more flat close
        "single_day": q.get("single_day", False), "single_day_pct": q.get("single_day_pct"),
        "gap": {"max1d": round(max(abs(cs[i] / cs[i - 1] - 1) for i in range(n - 30, n)) * 100, 1),
                "max1d_date": CAL[fi + max(range(n - 30, n), key=lambda i: abs(cs[i] / cs[i - 1] - 1))],
                "gain20": round((cs[-1] / cs[-21] - 1) * 100, 1)},
        "slope": round(q["slope"], 2), "L": q["L"], "W": q["W"],
        "vcp": elig[sym]["vcp"], "_vcpr": elig[sym]["vcp_raw"],
        "cert": st["cert"], "_certr": st["cert_raw"],
        "combo": st["combo"], "_combor": st["combo_raw"],
        "cert_c": {
            "broke": st["broke"], "H_mid": round(st["H_mid"], 2),
            "post_high": round(st["post_high"], 2),
            "retrace_pct": round(max(0.0, min(1.0, st["retrace"])) * 100, 1),
            "retrace_raw": round(st["retrace"] * 100, 1),
            "d_held": st["d_held"], "undercut": st["undercut"],
            "dv_ratio": round(u["dv_ratio"], 2),
            "contr": round(st["contr"], 2),
            "rs21_pct": round(u["rs21"] * 100, 1),
            "ma_flags": u["ma_flags"],
            "s": {k: round(v, 3) for k, v in (
                ("break", st["s_break"]), ("retr", st["s_retr"]), ("time", st["s_time"]),
                ("dv", u["s_dv"]), ("contr", st["s_contr"]), ("rs", u["s_rs"]), ("ma", u["s_ma"]))},
        },
        "hits": hits[sym],
        "hl": [[CAL[fi + i], round(c, 4)] for i, c in st["hl"]],
        "spark": {"closes": [round(c, 4) for c in cs[-60:]],
                  "ma": [round(x, 4) if x else None for x in (sma_series(cs, q["L"]))[-60:]],
                  "dates": CAL[fi + n - 60: fi + n],
                  "bots": spark_b},
        "est_bars": sorted(dd for dd, how in EST.get(sym, {}).items()
                           if how in ("official", "interp") and dd in CAL[fi + off: fi + n]),
        "est_bottom": [CAL[fi + i] for i, c in st["hl"] if EST.get(sym, {}).get(CAL[fi + i]) in ("official", "interp")],
        "deal": DEALS.get(sym),
    }

out = {"meta": {"last_date": LAST_DATE, "cal_first": CAL[0], "cal_last": CAL[-1],
                "n_days": len(CAL), "counts": stats_counts, "eligible": len(syms),
                "mkt_med_21_pct": round(mkt_med * 100, 2),
                "tiers": [{"key": k, "en": en, "zh": zh,
                           "lo": lo, "hi": (None if hi == float("inf") else hi)}
                          for k, en, zh, lo, hi in TIERS]},
       "pages": {}}

all_rows = {p: [row(s, q) for s, q in qual[p].items()] for p in PAGES}
# ---- deal-pinned / flat-tape exclusion (R8): names whose price is pinned by a pending
# cash acquisition (or otherwise dead-flat) score near-perfect VCP with zero breakout
# potential; they are pulled out of the ranking into their own box, ranks preserved.
if FLAT15 > 0:
    for s in struct:
        if s in DEALS: continue
        m0 = meta.get(s) or meta.get(norm(s)) or {}
        if is_fund(m0.get("name", s), m0.get("industry", "")) or cap_tier(m0.get("mcap", 0.0)) == "x":
            continue          # funds / no-cap instruments are excluded elsewhere
        cs15 = SER[s][1][-15:]
        rng = (max(cs15) - min(cs15)) / cs15[-1] * 100
        medret = statistics.median(abs(cs15[i] / cs15[i - 1] - 1) * 100 for i in range(1, len(cs15)))
        if rng < FLAT15 and medret < 0.25:
            DEALS[s] = {"auto": True, "range15": round(rng, 2),
                        "terms_zh": f"15日收盤區間僅 {rng:.2f}%、日均絕對變動 {medret:.2f}%（自動規則 A1：區間<{FLAT15:g}% 且中位日變動<0.25%）"}
            continue
        c60 = SER[s][1][-60:]
        jumps = [(abs(c60[i] / c60[i - 1] - 1), i) for i in range(1, len(c60))]
        jm, ji = max(jumps)
        post = c60[ji:]
        if jm >= 0.15 and len(post) >= 5 and (max(post) - min(post)) / post[-1] * 100 < 3.0:
            DEALS[s] = {"auto": True, "range15": round(rng, 2),
                        "terms_zh": f"單日跳升 {jm*100:.0f}% 後 {len(post)} 日區間 <3%（自動規則 A2：公布日後釘價）"}
deal_syms = set(DEALS)
deal_would = {}
for p in PAGES:
    for key, en, zh, lo, hi in TIERS:
        rows = sorted([r for r in all_rows[p] if r["tier"] == key], key=lambda r: -r["_combor"])
        for i, r in enumerate(rows, 1):
            if r["sym"] in deal_syms:
                deal_would.setdefault(r["sym"], {})[f"{p}{key}"] = i
deal_n = {str(p): sum(1 for r in all_rows[p] if r["sym"] in deal_syms) for p in PAGES}
deals_out = []
present = {r["sym"] for p in PAGES for r in all_rows[p] if r["sym"] in deal_syms}
for s in sorted(present, key=lambda s: min(deal_would.get(s, {"-": 999}).values())):
    wr = deal_would.get(s, {})
    r0 = next(r for p in PAGES for r in all_rows[p] if r["sym"] == s)
    deals_out.append(dict(DEALS[s], sym=s, name=r0["name"], close=r0["close"], tier=r0["tier"],
                          mcap=r0["mcap"], vcp=r0["vcp"], cert=r0["cert"], combo=r0["combo"],
                          would_ranks=wr))
for p in PAGES:
    all_rows[p] = [r for r in all_rows[p] if r["sym"] not in deal_syms]
print(f"併購套利／釘死價格剔除: {len(deals_out)} 隻 {[x['sym'] for x in deals_out]} | 各時間框 {deal_n}")
unknown = {str(p): sum(1 for r in all_rows[p] if r["tier"] in ("x", "f")) for p in PAGES}
unknown_syms = sorted({r["sym"] for p in PAGES for r in all_rows[p] if r["tier"] == "x"})
fund_syms = sorted({r["sym"] for p in PAGES for r in all_rows[p] if r["tier"] == "f"})
out["meta"]["unknown_cap"] = {"per_timeframe": unknown, "symbols": unknown_syms, "funds": fund_syms}
out["meta"]["class_dupes"] = CLASS_DUPES
print("封閉式基金／royalty trust（剔出分層頁）:", fund_syms)
print(f"未知市值（非普通股，已排除出分層頁）: {len(unknown_syms)} 隻 {unknown_syms} | 各時間框 {unknown}")
for p, (L, W, label) in PAGES.items():
    for key, en, zh, lo, hi in TIERS:
        rows = [r for r in all_rows[p] if r["tier"] == key]
        rows.sort(key=lambda r: -r["_combor"])
        pid = f"{p}{key}"
        out["pages"][pid] = {"L": L, "W": W, "label": label, "tier": key,
                             "tier_zh": zh, "tier_en": en,
                             "qualified": len(rows), "rows": rows[:50],
                             "all_ranks": {r["sym"]: i for i, r in enumerate(rows, 1)}}
        print(f"P{pid} ({label} · {zh}): qualified {len(rows)}, listing {min(50, len(rows))}")
    tot = sum(out["pages"][f"{p}{k}"]["qualified"] for k, _, _, _, _ in TIERS) + unknown[str(p)] + deal_n[str(p)]
    print(f"  守恆 P{p}: 三層 + 未知市值 {unknown[str(p)]} + 併購剔除 {deal_n[str(p)]} = {tot}")

PAGE_IDS = [f"{p}{k}" for p in PAGES for k, _, _, _, _ in TIERS]
listed = {}
PREF = {"4": 0, "3": 1, "5": 2, "2": 3}          # page-1 display row: 1個月 20MA first, 10MA last
for pid in sorted(PAGE_IDS, key=lambda x: (PREF[x[0]], x)):
    for i, r in enumerate(out["pages"][pid]["rows"], 1):
        listed.setdefault(r["sym"], dict(r, ranks={}))
for pid in PAGE_IDS:
    for i, r in enumerate(out["pages"][pid]["rows"], 1):
        listed[r["sym"]]["ranks"][pid] = i
p1 = []
for s, r in listed.items():
    score_raw = 0.4 * r["_vcpr"] + 0.4 * r["_certr"] + 0.2 * (hits[s] / 4 * 100)
    r["score"] = round(score_raw, 1)
    r["_scorer"] = score_raw
    p1.append(r)
p1.sort(key=lambda r: -r["_scorer"])
for r in p1:
    del r["_scorer"]
for pid in PAGE_IDS:
    for rr in out["pages"][pid]["rows"]:
        rr.pop("_vcpr", None); rr.pop("_certr", None); rr.pop("_combor", None)
for r in p1:
    r.pop("_vcpr", None); r.pop("_certr", None); r.pop("_combor", None)
out["page1"] = p1
out["meta"]["page_ids"] = PAGE_IDS
out["meta"]["deal_pinned"] = {"per_timeframe": deal_n, "symbols": [x["sym"] for x in deals_out], "flat15": FLAT15}
out["deal_pinned"] = deals_out
out["meta"]["data_fix"] = {k: META8[k] for k in ("missing_sessions", "fill_stats", "vol_ratio_0827",
                                                    "official_source", "real_source") if k in META8}
out["meta"]["diag"] = {"eligible": sorted(syms), "struct": sorted(struct), "undercut_gate": sorted(GATED),
                       "qual": {str(p): sorted(qual[p]) for p in PAGES}}
print(f"P1 summary: {len(p1)} distinct tickers across {len(PAGE_IDS)} tier pages")
json.dump(out, open(os.environ.get("OUT", f"{SCRATCH}/screen_results8.json"), "w"), ensure_ascii=False)
print("counts:", stats_counts, "eligible:", len(syms), "with structure:", len(struct), "| gated (close below last low):", len(GATED))
tier_n = {}
for r in p1:
    tier_n[r["tier"]] = tier_n.get(r["tier"], 0) + 1
print("distinct by tier:", tier_n)
