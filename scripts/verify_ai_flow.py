...
F=json.load(open(f"{SCR}/ai/flow.json")); M=F["meta"]; DAYS=M["days"]; rows=[r for r in F["rows"] if r.get("days")]
...
htmlf=sorted(glob.glob(f"{SCR}/AI_Sector_watchlist_R1.00_*.html"))[-1]
