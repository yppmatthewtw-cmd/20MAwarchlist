...
④ <b>數據終點 {DAYS[-1]}（美東週{WD[datetime.date.fromisoformat(DAYS[-1]).weekday()]}收盤）</b>：日線鏡像（09-05 00:11 UTC 提交）對 09-04 只有成交量、Nasdaq 快照最新一筆係 09-04 盤中，
故 09-04 整日 OHLCV 來自 <b>Yahoo Finance</b>（GitHub Actions runner 以 yfinance 拉取未調整日線：{esc("、".join(M.get("yahoo_files", [])))}）。
Yahoo 與日線鏡像視窗內共同收盤中位偏差 {XN.get("median_pct", 0):.4f}%；與快照序列逐日中位偏差 0.0000%。每隻股票以日線鏡像為主、Yahoo 補其未有嘅交易日；基準不符者（{esc("、".join(basis))}）整段改用 Yahoo。<br>
...
open(f"{SCRATCH}/ai5/artifact_fragment.html", "w", encoding="utf-8").write(html_doc)
