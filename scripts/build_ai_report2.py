...
F = json.load(open(f"{SCRATCH}/ai/flow2.json"))
B = json.load(open(f"{SCRATCH}/ai/flow.json"))   # R1.00, for the change summary only
...
OUTNAME = f"AI_Sector_watchlist_R2.00_claudeopus5high_{STAMP}.html"
WD = "一二三四五六日"
...
def day_cell(r, d):
    x = r["days"][d]
    nv = '<span class="nvw" title="當日成交量尚未公布，量能項設為中性">量?</span>' if x.get("novol") else ""
...
COLW_MAIN = [40, 206, 116, 92, 88] + [76] * len(DAYS) + [90, 80, 78, 92, 92, 186, 122, 206]
...
nv = M.get("novol_note", {})
upd = f"""
<div class="upd"><h3>本版更新（R2.00 vs R1.00）</h3><ul>
<li><b>數據推進一個交易日</b>：計分視窗由 {BDAYS[0]} → {BDAYS[-1]} 改為 <b>{DAYS[0]} → {DAYS[-1]}</b>
（新增 {esc("、".join(NEWD))}，移出 {esc("、".join(GONED))}）。{DAYS[-1]} 收盤取自 09-03 盤中快照嘅
<code>price − price_change</code>（即官方前收），已與日線鏡像 1,498 隻交叉核對，中位偏差 0.0000%。</li>
<li><b>成交量缺口處理</b>：09-03 只有盤中快照，其成交量係當日 partial，故 <b>{DAYS[-1]} 只有日線鏡像覆蓋嘅股票有真實成交量</b>；
其餘股票當日<b>量能項 B 設為中性 0</b>（唔用估算量冒充），受影響格內標「量?」——本版 {len([1 for r in live if r["novol_days"]])} 個小群組有此情況。</li>
<li><b>新增「穩健分」</b>：原始分數對樣本細嘅小群組噪音大（C1 得 MU 一隻、D2/C4/D6/B4 各一隻）。
穩健分 = 5日 z × √(n/(n+2)) 後再取百分位，樣本愈細愈向中性收斂；<b>兩個分數都可排序</b>，可對照睇。</li>
<li><b>新增「資金強度」</b>：淨額估算 ÷ 同期成交額（%），去除規模效應 —— 大群組（如 G3 11 隻）淨額天然大，
呢欄先睇得出「每一蚊成交額入面有幾多係買方主導」。</li>
<li><b>新增「可信度」與「廣度」</b>：可信度按美股樣本數（≤2 低／3 中／≥4 高）；
廣度＝籃子內超額報酬為正嘅成分股比例，用嚟分辨「全組齊升」定「一隻股拉起」。</li>
<li><b>名次變動</b>：{len(moved)} 個小群組名次有變 —— 上升最多 {esc("、".join(up3)) or "無"}；
下跌最多 {esc("、".join(dn3)) or "無"}。可計算 {len(live)} 組、未能計算 {len(dead)} 組，與 R1.00 相同。</li>
<li><b>儀表板落差擴大</b>：RS 象限仍為儀表板 2026-08-27 數值，本版資金流向已到 {DAYS[-1]}，相差 4 個交易日，
第 4 頁背離分析請以此為前提。</li>
</ul></div>"""
...
⑦ <b>成交量缺口</b>：{esc(DAYS[-1])} 只有日線鏡像覆蓋嘅股票有真實成交量，其餘股票該日量能項設為中性（格內標「量?」），
唔以估算量冒充；受影響嘅小群組見「趨勢」欄下方標示。<br>
⑧ <b>儀表板象限</b>（領先／改善／轉弱／落後）同 RS、RS動能係<b>儀表板 8/27 收盤</b>嘅數值，
本表資金流向係 <b>{DAYS[0]} → {DAYS[-1]}</b>（相差 4 個交易日），兩者時點唔同，正好用嚟睇背離 —— 見第 4 頁。
</div>"""
...
④ <b>數據終點 {DAYS[-1]}（美東星期三收盤）</b>：日線鏡像已推送完整 9/2 日線（1,503 隻），
9/3 仍只得一半（1,003 隻有收盤、500 隻淨得成交量），故 9/3 未納入。全宇宙 9/2 收盤由 09-03 盤中快照嘅
<code>price − price_change</code> 取得（6,815 隻），與日線鏡像交叉核對中位偏差 0.0000%（1,498 隻，僅 2 隻 &gt;0.5%）。
視窗內 08-31 仍屬快照缺日，非日線鏡像覆蓋嘅股票以官方收盤補回、成交量取前後平均；R1.00 視窗內嘅 08-26（同屬估算日）
已隨視窗推進移出。<br>
