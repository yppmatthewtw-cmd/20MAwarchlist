...
NEWS = json.load(open(f"{SCRATCH}/news7.json")) if os.path.exists(f"{SCRATCH}/news7.json") else {}
...
OUTNAME = f"20MA_uptrend_watchlistGit_R7.00_claudeopus5max_{STAMP}.html"
...
c = M["counts"]
UNIVERSE_LINE = (f'全美上市普通股掃描 —— 快照涵蓋 {c["total"]:,} 隻 · 有 {M["last_date"]} 收盤報價 {c["current"]:,} 隻 · '
...
<h2>篩選規則（R7 · 數據更新至 9/1 收盤 · 全美掃描三層分榜）</h2>
① <b>Universe：{esc(UNIVERSE_LINE)}</b>。<br>
② <b>市值分層</b>：<b>大型股 ≥$10B</b> · <b>中型股 $2B–$10B</b> · <b>小型股 &lt;$2B</b>（含通過流動性門檻嘅微型股）。每個時間框各自分三層，<b>每層獨立取 top 50</b>，細價股唔會被大價股擠走。快照無市值數據嘅非普通股證券（優先股／存託股、SPAC、封閉式基金）唔會當成小型股，已剔出分層頁 —— 本次 {esc(len(M.get("unknown_cap", {}).get("symbols", [])))} 隻（{esc("、".join(M.get("unknown_cap", {}).get("symbols", [])))}）。<br>
...
<div class="card foot">
<h2>備註 · 數據 lineage</h2>
① <b>覆蓋範圍（R7 回復全美掃描）</b>：快照鏡像已恢復運作，本版重新覆蓋全美上市普通股（9/1 快照 7,153 行，重建出 7,396 條序列、6,874 隻有 9/1 收盤），唔再受 R6 嘅 S&amp;P 1500 限制。價格未除息調整。<br>② <b>數據來源（雙來源合成）</b>：主線為 zyhe16/top-us-stock-tickers 嘅每日 Nasdaq 快照，逐 commit 重建收盤/成交量序列，本表取 {M["n_days"]} 個交易日（{M["cal_first"]} → {M["cal_last"]}）；該鏡像 8/31 因排程故障無快照，<b>已用 natezone/market-tracker 嘅真實日線補回 1,500 隻</b>，其餘以前值填補。<b>9/1 收盤已與獨立來源（Yahoo 日線）交叉核對：抽樣 30 隻全部吻合，偏差 0.000%</b>。另註：日線鏡像當時只出到盤中快照，抽取程式設有「收市後 commit」防護，會自動剔除未完成嘅當日 bar。<br>③ 類別：Nasdaq 分類＋GICS（S&amp;P 500）；交易所：irachex/open-stock-data。VCP、確定性 7 項及排名邏輯沿用 R3–R5 定義（未改動），已經 R3 階段 6 組、R5 分層 1 組獨立代理人對抗性驗證：12 頁排名、三層守恆、分層歸屬、與前版指標一致性、總表聯集及名次映射全部 PASS；驗證揪出「無市值數據被當成小型股」一項缺陷，已於本版修正（改為剔出分層頁並喺上方列明）。<br>
...
html_doc = f"""<title>20MA Uptrend Watchlist R7</title>
<style>{css}</style>
<div class="wrap">
<h1>20MA Uptrend Watchlist R7（全美掃描 × 市值分層 × VCP × 底部確定性）</h1>
<div class="sub">數據至 <b>{M["last_date"]}</b>（美東週二）收盤 · 全美掃描 {c["liq"]:,} 隻合資格 · 12 個分層榜（4 時間框 × 大／中／小型股）＋總表 · 🔥 熱炒 news-driven 催化劑獨立成欄 · 固定深色版面</div>
{rules_html}
<nav class="nav">
...
(function() {{
  ...
  function show() {{
    ...
    tierrow.hidden = (g === '1');
    syncSort();
    try {{ localStorage.setItem('r7nav', g + '|' + t); }} catch (e) {{}}
  }}
  var sortState = {{}};
  ...
  try {{
    var sv = localStorage.getItem('r7nav');
    if (sv) {{
      var parts = sv.split('|');
