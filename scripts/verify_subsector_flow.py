...
F=json.load(open(f"{SCR}/sub/flow.json")); M=F["meta"]; DAYS=M["days"]; rows=[r for r in F["rows"] if r.get("days")]
S8=pickle.load(open(f"{SCR}/series8.pkl","rb")); CAL=S8["cal"]; SER=S8["series"]
bad=[]
# 1) structural
...
for target in (1, 111, 45):
    ...
    for day in DAYS:
        ...
        for t in r["basket"]:
            ...
            base=statistics.median(data[d][3]*data[d][4] for d in BASE if d in data)
            dv=c*v; rvol=dv/base
            A=math.tanh((c/pc-1-mkt[day])/0.02); B=max(-1,min(1,math.log2(max(.25,min(4,rvol)))/2))
...
htmlf=sorted(glob.glob(f"{SCR}/SubSector_flow_watchlist_R1.00_*.html"))[-1]
