#!/usr/bin/env python3
"""Sub-Sector 資金流向 Watchlist R11.00 — 111 sub-sectors x 5 sessions of money-flow scores (Yahoo second source).
...
VER = "R11.00"
OUTNAME = f"SubSector_flow_watchlist_{VER}_claudeopus5high_{STAMP}.html"
WD = "一二三四五六日"
...
　<code>B 量能 = log₂(當日<b>成交股數</b> / 前 20 日中位<b>成交股數</b>) ÷ 2</code>，截於 ±1 —— 放量或縮量；<br>　（R7 起用股數而非成交額：分子分母同用價格會令價格漂移混入參與度，實測 6.7% 嘅「股票×交易日」因而偏差超過 0.10。成交額仍然負責籃子加權同淨額估算。）<br>
...
<span>格內數字＝當日 0–100 分（橫向百分位）· 滑鼠停留睇 z 值／籃子報酬／量能倍數{"　·　淨額估算旁嘅 ≈ ＝ 含未結算成交量嘅交易日，屬臨時值（分數與名次唔受影響）" if PROV_DAYS else ""}</span></div>
</section>''')
...
<b>淨額估算 p95 差 {PE.get("mfd_p95_pct", 0):.1f}%、最大 {PE.get("mfd_max_pct", 0):.1f}%</b>，故該等金額旁標 <b>≈</b>。<br>
③ <b>來源合併規則</b>：每隻股票以日線鏡像為主、Yahoo 補其未有嘅交易日，但<b>鏡像必須喺 {M.get("merge_since", "—")} 之後仍有數據</b>先可以合併，
且基準核對只計視窗內嘅共同交易日（最近 15 個，收盤中位偏差 ≤0.5%）；停更太耐（本次 {len(stale)} 隻：{esc("、".join(stale))}）
或基準不符（{esc("、".join(basis))}）者，整段改用 Yahoo 已回溯調整嘅歷史。快照序列只作對照，唔入計分。<br>
...
html_doc = f"""<title>Sub-Sector 資金流向 Watchlist R11</title>
<style>{css}</style>
<div class="wrap">
