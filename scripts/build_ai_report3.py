...
F = json.load(open(f"{SCRATCH}/ai/flow3.json"))
...
OUTNAME = f"AI_Sector_watchlist_R3.00_claudeopus5high_{STAMP}.html"
WD = "一二三四五六日"
...
def basket_cell(r):
    ...
    mtag = (f'<span class="miss" title="美股上市但鏡像歷史不足 5+20 個交易日，未計入">數據不足 {esc("、".join(miss))}</span>'
            if miss else "")
    non = r.get("nonus") or []
...
nv = M.get("novol_note", {})
upd = f"""
<div class="upd"><h3>本版更新（R3.00 vs R2.00）</h3><ul>
<li><b>新增「成分股 · 按資金流向排序」欄</b>（喺「AI 小群組」同「大分類」之間）：每個小群組嘅美股／ADR 成分股
逐隻計算 5 日資金流向分，<b>由流入最多排到流出最多</b>，並以<b>綠底＝資金流入、紅底＝資金流出</b>標示
（深淺代表強度，灰＝中性）。代號右邊細字即該股嘅 5 日流向分；滑鼠停留可睇淨額估算同 5 日報酬。</li>
<li><b>原「成分股（美股／ADR）」欄改為「未計入成分股」</b>：成分股名單已移到新欄，右邊只保留<b>冇計入</b>嘅部分
（鏡像數據不足、或非美股上市而按指示排除），避免重複。</li>
<li><b>個股分數同群組分數同一條公式</b>：逐日 f =（0.70×方向 + 0.30×收位）×（1 + 0.50×量能），
再用同一組近日較重嘅權重加權平均，所以群組排名同欄內個股排名可以直接對照 ——
群組排前但欄內見到紅底股票，即係該組升勢集中喺少數成分股。</li>
<li><b>數據與計分邏輯未變</b>：仍為 {DAYS[0]} → {DAYS[-1]} 五個交易日、只計美股及 US ADR，
可計算 {len(live)} 組、未能計算 {len(dead)} 組，排名與 R2.00 一致。</li>
</ul></div>"""
