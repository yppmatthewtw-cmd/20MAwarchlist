...
rep('"""Sub-sector money-flow scoring — R6.00 engine (111 sub-sectors of the R2 heat-map workbook).',
    '"""AI money-flow scoring — R7.00 engine (41 AI 小群組 of the Dashboard R15.6 classification).\n\n'
    'Identical scoring math to the Sub-Sector watchlist R6.00 engine (Yahoo second source); only\n'
...
rep('json.dump(out, open(os.environ.get("OUT_JSON", f"{SCR}/sub6/flow6.json"), "w"), ensure_ascii=False)',
    ...
    'json.dump(out, open(os.environ.get("OUT_JSON", f"{SCR}/ai7/flow7.json"), "w"), ensure_ascii=False)')
...
open(f"{S}/ai7/flow7.py","w",encoding="utf-8").write(src); print("ai7/flow7.py written")
