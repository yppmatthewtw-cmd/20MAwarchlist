...
def series_snap(sym):
    ...
    return {d: (None, None, None, cs[idx[d]], vs[idx[d]]) for d in LAST if d in idx}
...
def tick_flow(sym):
    ...
    base_dv = [data[d][3] * data[d][4] for d in BASE if d in data]
    ...
    for day in DAYS:
        ...
        exret = ret - MKT[day]
        dv = c * v
        rvol = dv / med_dv if med_dv > 0 else 1.0
        A = math.tanh(exret / 0.02)
        B = max(-1.0, min(1.0, math.log2(max(0.25, min(4.0, rvol))) / 2))
        ...
        out["days"][day] = {"ret": ret, "exret": exret, "dv": dv, "rvol": rvol,
...
json.dump(out, open(f"{SCR}/sub/flow.json", "w"), ensure_ascii=False)
print("\nTOP 12 inflow:")
for r in live[:12]:
